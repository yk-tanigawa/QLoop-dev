...
int constructBitTableColumn(const int *h_i, 
  ...
  unsigned int buf = 0;
  {
    *x = calloc_errchk(nkmerpairs, sizeof(unsigned int *), "calloc x");
    for(lm = 0; lm < nkmerpairs; lm++){
      (*x)[lm] = calloc_errchk(((nHic + intBits - 1) / intBits), 
			       sizeof(unsigned int), "calloc x[lm]");
    }
  }
